...
// Rect represents properties from a rectangle (like an Image) inside a cell.
type Rect struct {
	// Left is the space between the left cell boundary to the rectangle, if center is false.
	Left float64
	// Top is space between the upper cell limit to the rectangle, if center is false.
	Top float64
	...
	// Center define that the rectangle will be vertically and horizontally centralized.
	Center bool
}
